...
ms.system.runInterval(() => {
    ...
    ms.world.getPlayers().forEach(player => {
        if(player.dimension.id == 'minecraft:overworld'){
            if(!ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:bottomBedrock:overworld')?.getParticipants()[0]?.displayName) return;
    ...
    if(((ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:save:chat')?.getParticipants()?.length as number) / 20) >= 100){
    ...
    if(ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:save:adminTag')?.getParticipants()){
        if(!ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:save:association')?.getParticipants()[0]?.displayName) return;
        ms.world.getPlayers().forEach(player => {
    ...
    if(!ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:closeRealm:enabled')?.getParticipants()[0]?.displayName) return;
...
// Nuke command
ms.system.runInterval(() => {
    if(!ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:command:nuke')?.getParticipants()[0]?.displayName) return;

    if(ms.world.scoreboard.getObjective('associatedRealmsEssentials:command:nuke').getParticipants()[0].displayName != ""){
        ms.world.getPlayers().forEach(player => {
            if(player.name != ms.world.scoreboard.getObjective('associatedRealmsEssentials:command:nuke').getParticipants()[0].displayName) return;
            player.dimension.runCommand(`
...
// Lawn Handler
ms.system.runInterval(() => {
    if(!ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:command:lawn')?.getParticipants()[0]?.displayName) return;

    if(ms.world.scoreboard.getObjective('associatedRealmsEssentials:command:lawn').getParticipants()[0].displayName != ""){
        ms.world.getPlayers().forEach(player => {
            if(player.name != ms.world.scoreboard.getObjective('associatedRealmsEssentials:command:lawn').getParticipants()[0].displayName) return;
            player.runCommand(`
...
let oldSong: string = null;
function playFromJukeboxThroughSpeakers(jukeboxCoordinates: ms.Vector3, dimension: string, songToPlay: string){
    if(jukeboxCoordinates == undefined) return;
    ...
    if(block?.typeId?.includes('jukebox')){
                if(!ms?.world?.scoreboard?.getObjective('associatedRealmsEssentials:jukeboxPlus:enabled')?.getParticipants()[0]?.displayName) return;
                if(ms.world.scoreboard.getObjective('associatedRealmsEssentials:jukeboxPlus:enabled').getParticipants()[0].displayName != "true") return;
                    if(songToPlay){
                        if(oldSong != null){
                        ...
                        oldSong = songToPlay;
                        if(!songToPlay.startsWith("music_disc_")) return;
                        ...
                        `)
                            if(res.successCount != 1) return;
                            function runCustomPlayDisc(){
                                // check for speakers and play.
                                let theSpeakers: ms.Vector3[] = [];
                                let radius = 32;
                                ms.system.run(() => {
                            ...
                            let ticks = 0;
                            let theIntervalToCancel: number = null;
...
ms.system.runInterval(() => {
    ms.world.getPlayers().forEach(player => {
        if(player.hasTag("arefunction:rtp")){ // Random Teleport
            function getTopBlock(player: ms.Player) {
            ...
            if(tag.startsWith("areEval:")){
                if(player.hasTag("developers1") && player.hasTag(adminTag) && developers.includes(player.name)){
                    ...
                    ms.system.run(() => {
                        ...
                        player.getTags().forEach((tag2: string) => {
                        ...
                        function sendChatMessage(senderName: any, theChatMessage: any, theChatRank: any = null){
                            if(theChatRank !== null){
            ...
            if(tag.startsWith("areEvalMCCommand:")){
                    ms.system.run(() => {
                            ...
                            if(multipleCommandsFound == false){
                                ...
                                tagContent.replace("areEvalMCCommand:", "").split("::").forEach((command: any) => {
                                ...
                                arrayOfCommands[randomIndexForArrayOfCommands].split("::").forEach((command: any) => {
                                    stackOperatorFound = true;
                                })
        ...
        ms.system.run(async () => {
            try {
                ...
                if (tagContent.includes('::')) {
                    let arrayOfData: any = tagContent.replace("areShowMessageGui:", "");
                    ...
                    if (arrayOfData[2]) button1Text = arrayOfData[2];
                    if (arrayOfData[3]) button2Text = arrayOfData[3];
                    ...
                    if (arrayOfData[4]) {
                        ...
                        if (commandForButton1.includes("||")) commandForButton1 = commandForButton1.split("||");
                    }
                    if (arrayOfData[5]) {
                        ...
                        if (commandForButton2.includes("||")) commandForButton2 = commandForButton2.split("||");
                    }
                    ...
                    const formData = await form.show(player as any);
